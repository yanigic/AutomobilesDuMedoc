...
const fetchData = async () => {
  try {
    ...
    projects.value = content.body.filter(project => project.title && project.img).map((project) => {
...
<template>
  <div  >
    <div class="heroSingleProject-container" >
      <img  v-for="(project, index) in projects" :key="index"  class="img-singleProject" :src="project.img" />
      <h3  v-for="(project, index) in projects" :key="index"  class="title-singleProject">
        {{ project.title }}
      </h3>
      <img class="svg-heroSingleProject" src="../assets/img/svg/arrowDown.svg" alt="Navigate Down"/>
    </div>
  </div>
</template>
